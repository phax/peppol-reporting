...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
  ...
  <p id="about">
    ...
    History:
      v1.0.5
        2024-02-29, Philip Helger - Improved execution speed of Schematron
      v1.0.4
  ...
  <xsl:function name="f:schemeAndValue" as="xs:string">
    <xsl:param name="node" />
  ...
  <xsl:key name="key-tp" 
           match="/tsr:TransactionStatisticsReport/tsr:Subtotal[normalize-space(@type) = 'PerTP']" 
           use="f:schemeAndValue(tsr:Key[normalize-space(@metaSchemeID) = 'TP'])" />  

  <xsl:key name="key-spdtpr" 
           match="/tsr:TransactionStatisticsReport/tsr:Subtotal[normalize-space(@type) = 'PerSP-DT-PR']" 
           use="concat (f:schemeAndValue(tsr:Key[normalize-space(@metaSchemeID) = 'SP']),'::',
                        f:schemeAndValue(tsr:Key[normalize-space(@metaSchemeID) = 'DT']),'::',
                        f:schemeAndValue(tsr:Key[normalize-space(@metaSchemeID) = 'PR']))" />  

  <xsl:key name="key-spdtprcc" 
           match="/tsr:TransactionStatisticsReport/tsr:Subtotal[normalize-space(@type) = 'PerSP-DT-PR-CC']" 
           use="concat (f:schemeAndValue(tsr:Key[normalize-space(@metaSchemeID) = 'SP']),'::',
                        f:schemeAndValue(tsr:Key[normalize-space(@metaSchemeID) = 'DT']),'::',
                        f:schemeAndValue(tsr:Key[normalize-space(@metaSchemeID) = 'PR']),'::',
                        f:schemeAndValue(tsr:Key[normalize-space(@schemeID) = 'SenderCountry']),'::',
                        f:schemeAndValue(tsr:Key[normalize-space(@schemeID) = 'ReceiverCountry']))" />  
  ...
  <pattern id="default">
    ...
    <rule context="/tsr:TransactionStatisticsReport/tsr:Subtotal[normalize-space(@type) = 'PerTP']">
      ...
      <assert id="SCH-TSR-06" flag="fatal" test="every $tp in (tsr:Key[normalize-space(@metaSchemeID) = 'TP']) satisfies
                                                   count(key('key-tp', f:schemeAndValue($tp))) = 1"
    ...
    <rule context="/tsr:TransactionStatisticsReport/tsr:Subtotal[normalize-space(@type) = 'PerSP-DT-PR']">
      ...
      <!-- Global uniqueness check per Key -->
      <assert id="SCH-TSR-10" flag="fatal" test="every $sp in (tsr:Key[normalize-space(@metaSchemeID) = 'SP']),
                                                       $dt in (tsr:Key[normalize-space(@metaSchemeID) = 'DT']),
                                                       $pr in (tsr:Key[normalize-space(@metaSchemeID) = 'PR']) satisfies
                                                   count(key('key-spdtpr', concat (f:schemeAndValue($sp),'::',
                                                                                   f:schemeAndValue($dt),'::',
                                                                                   f:schemeAndValue($pr)))) = 1"
    ...
    <rule context="/tsr:TransactionStatisticsReport/tsr:Subtotal[normalize-space(@type) = 'PerSP-DT-PR-CC']">
      ...
      <!-- Global uniqueness check per Key -->
      <assert id="SCH-TSR-14" flag="fatal" test="every $sp in (tsr:Key[normalize-space(@metaSchemeID) = 'SP']),
                                                       $dt in (tsr:Key[normalize-space(@metaSchemeID) = 'DT']),
                                                   ...
                                                   count(key('key-spdtprcc', concat (f:schemeAndValue($sp),'::',
                                                                                     f:schemeAndValue($dt),'::',
                                                                                     f:schemeAndValue($pr),'::',
                                                                                     f:schemeAndValue($sc),'::',
                                                                                     f:schemeAndValue($rc)))) = 1"
