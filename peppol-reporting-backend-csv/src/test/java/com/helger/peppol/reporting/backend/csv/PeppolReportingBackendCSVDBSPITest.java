...
/**
 * Test class for class {@link PeppolReportingBackendCSVSPI}.
 *
 * @author Philip Helger
 */
public final class PeppolReportingBackendCSVDBSPITest
{
  private static final Logger LOGGER = LoggerFactory.getLogger (PeppolReportingBackendCSVDBSPITest.class);
  ...
  public void testBasic () throws PeppolReportingBackendException
  {
    ...
    final ESuccess eSuccess = PeppolReportingBackend.withBackendDo (ConfigFactory.getDefaultConfig (), aBackend -> {
      ...
      for (int i = 0; i < nReportItems; ++i)
      {
        ...
        final ICommonsList <String> aList = PeppolReportingBackendCSVSPI.asCSV (aItem);
        final PeppolReportingItem aItem2 = PeppolReportingBackendCSVSPI.asItem (aList);
      ...
      assertTrue (aStoredItems.size () + " remaining of " + nReportItems + ": " + aStoredItems,
                  aStoredItems.isEmpty ());
    });
